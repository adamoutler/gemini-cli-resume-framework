...
def run_workflow(jd_name, sentinel_only=False, skip_existing=False, notes=None):
    ...
    for qc_attempt in range(MAX_RETRIES):
        ...
        if val_result.returncode != 0:
            ...
            fixed_raw = call_gemini(fixer_persona, fix_task, session_id=fixer_session)
        ...
        review_raw = call_gemini(reviewer_persona, review_task, session_id=reviewer_session)
    ...
    cl_text = call_gemini(cl_persona, cl_task, session_id=cl_session)
    ...
    for cl_fix_attempt in range(3):
        if cl_text and cl_text != unidecode(cl_text):
            ...
            cl_text = call_gemini(cl_persona, cl_fix_task, session_id=cl_session)
        else:
            break
    else:
