...
sys.path.append(os.path.join(PROJECT_ROOT, "Codex_Tasks", "Maintenance"))
try:
...
def call_codex(system_prompt, user_input, max_retries=3):
    """Calls Codex API with exponential backoff."""
    full_prompt = f"{system_prompt}\n\n--- INPUT DATA ---\n{user_input}"
    cmd = ["codex", "exec", "--model", "gpt-5.4", "--color", "never", "-"]
    
    # Try to use master session if available in env
    session_id = os.environ.get('MASTER_SESSION_ID')
    if session_id:
        cmd = ["codex", "exec", "resume", "--model", "gpt-5.4", session_id, "-"]
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "429" in err_msg or "resource" in err_msg or "exhausted" in err_msg or result.returncode != 0:
                ...
                log("WARN", f"Codex API Error (Attempt {attempt+1}/{max_retries}). Backing off for {wait_time}s...")
                time.sleep(wait_time)
                continue
            
            log("ERROR", f"Codex CLI failed: {result.stderr}")
            return None
...
class PageLimitEnforcer:
    ...
    def get_ai_exclusion_recommendations(self, overflow_pages):
        ...
        raw_response = call_codex(system_prompt, task)
    ...
    def micro_prune_orphans(self, pdf_path, margin, scale):
        ...
        for orphan in orphans:
            ...
            if match:
                ...
                shortened = call_codex(system_prompt, task, max_retries=2)
