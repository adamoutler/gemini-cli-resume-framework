...
from utils.context_loader import create_codex_context_session, load_persona, resume_codex_session
...
# Auto-set CODEX_PROJECT_TMP_DIR if not set
if "CODEX_PROJECT_TMP_DIR" not in os.environ:
    unique_run_id = uuid.uuid4().hex[:8]
    generated_tmp_dir = f"/tmp/codex_audit_{unique_run_id}"
    os.environ["CODEX_PROJECT_TMP_DIR"] = generated_tmp_dir
    os.makedirs(generated_tmp_dir, exist_ok=True)
else:
    os.makedirs(os.environ["CODEX_PROJECT_TMP_DIR"], exist_ok=True)
...
def init_session(context_text):
    ...
    try:
        response = create_codex_context_session(context_text, model=MODEL)
        if response["returncode"] == 0:
            return response.get("thread_id")
        log(f"Session initialization failed: {response.get('stderr')}")
        return None
...
def init_session_pool(context_text, count, parent_session_id=None):
    """Initializes a master Codex restore point, or reuses one, for worker prompts."""
    ...
    log(f"Master Session Ready: {master_id}. Preparing {count} worker restore keys...")
    ...
    for i in range(count):
        ...
        # Codex restore keys are resumed directly; no local session files are copied.
        ...
        if worker_id_result:
            ...
            log(f"  - Worker {i+1}/{count} Failed to prepare restore key.")
...
def call_agent(prompt, session_id=None):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            log(f"Codex CLI Error: {stderr}")
            return None

        except subprocess.TimeoutExpired:
            log(f"Codex API Timeout (Attempt {attempt+1}/{MAX_RETRIES}). Backing off...")
            if attempt == MAX_RETRIES - 1:
                break
