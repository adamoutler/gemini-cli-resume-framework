    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if "429" in err_msg or "resource" in err_msg or "exhausted" in err_msg or result.returncode != 0:
                ...
                print(f"[WARN] Codex API Error (Attempt {attempt+1}/{MAX_RETRIES}). Backing off for {wait_time}s...")
            ...
            print("subprocess.TimeoutExpired: Codex CLI hung.")
            continue
        except subprocess.TimeoutExpired:
            print("WARN" if "print" in globals() else "print", f"Codex API Timeout (Attempt {attempt+1}/{MAX_RETRIES}). Backing off...")
...
def generate_cover_letter(resume_path, jd_path):
    ...
    cl_text = call_codex(cl_persona, cl_input)
