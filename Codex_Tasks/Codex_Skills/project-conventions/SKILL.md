...
description: General project rules and architectural documentation for Codex_Tasks. Use this when modifying files in Codex_Tasks, debugging the resume orchestrator, or editing automation scripts.
---

# Project Conventions & Codex_Tasks Architecture
...
## Codex_Tasks Architecture

**Trigger:** Activate this skill when the user asks for a task which requires you to modify, fix, or debug any script, persona, data, or file in `Codex_Tasks/` or the Orchestrator.

### Protocol: Editing Codex Tasks
If you modify any scripts or logic within the `Codex_Tasks/` directory:
...
### Entry Point
- **Script:** `Codex_Tasks/Orchestrator/resume_orchestrator.py`
- **Function:** `run_workflow()` governs the entire pipeline.

### Phase 0: Setup & Initialization
- **Goal:** Prepare the LLM session and data context.
- **Key Files:**
  - `Codex_Tasks/Orchestrator/utils/context_loader.py`: Recursively reads `cv-data/*.md` and structures it by schema section (Work, Skills, etc.).
  - `Codex_Tasks/Orchestrator/utils/session_manager.py`: Handles `init_master_session` (loads context once) and `fork_session` (creates lightweight workers for subsequent tasks).
...
1.  **Schema Validation:**
    -   **Tool:** `Codex_Tasks/Resume_Audit/validate_schema.py`
...
2.  **Forensic Audit:**
    -   **Tool:** `Codex_Tasks/Resume_Audit/audit_content_new.py`
...
- **Tool:** `Codex_Tasks/Maintenance/enforce_page_limit.py`
- **Logic:**
...
- **Tools:**
  ...
  -   `Codex_Tasks/Format_Conversion/html_to_pdf.py`: Renders HTML to PDF using Playwright (headless Chrome).
  -   `Codex_Tasks/Format_Conversion/append_data_to_pdf.py`: Appends the full `cv-data` as an appendix to the PDF.
- **Cover Letter:**
...
- **`Codex_Tasks/Format_Conversion/`**:
    ...
    ### 1. The Core: `Codex_Tasks/Orchestrator/`
    ...
    *   **`utils/session_manager.py`**: **All Phases**. Manages `~/.codex/sessions/` sessions, creating the Master Session and forking Worker Sessions for parallel tasks.
    *   **`personas/*.md`**: Loaded into memory and sent to Codex to "program" the agent for each phase.
    
    ### 2. Validation (Phase 3): `Codex_Tasks/Resume_Audit/`
    *   **`validate_schema.py`**:
    ...
    *   **`audit_content_new.py`**:
        ...
        *   **Mechanism:** `subprocess`.
        *   **Role:** The "Forensic Auditor." Spins up its *own* pool of Codex threads (via `session_manager`) to verify claims against `cv-data`. Reads `known_regressions.json` for rule checks.
    
    ### 3. Formatting & Layout (Phase 4): `Codex_Tasks/Maintenance/`
    *   **`enforce_page_limit.py`**:
    ...
    ### 4. Artifact Generation (Phase 5): `Codex_Tasks/Format_Conversion/`
    *   **`html_to_pdf.py`**:
    ...
    *   **`append_data_to_pdf.py`**:
        *   **Called By:** Orchestrator (Phase 5).
        ...
        These scripts exist in `Codex_Tasks` but are **NOT** part of the `resume_orchestrator.py` pipeline. They are intended for manual use or specific one-off tasks.
        
        - **`Codex_Tasks/Orchestrator/generate_cover_letter_only.py`**: Generates a cover letter for an existing resume/JD pair without regenerating the resume itself.
        - **`Codex_Tasks/Orchestrator/generate_interview_prep.py`**: Generates an interview preparation dossier based on the JD and resume.
        - **`Codex_Tasks/Resume_Audit/fix_resume_structure.py`**: A manual utility to repair broken JSON structure if the orchestrator fails catastrophically.
        - **`Codex_Tasks/Format_Conversion/json_to_md.py`**: Converts a JSON resume to Markdown format (useful for debugging or feeding back into LLMs manually).
        - **`Codex_Tasks/Maintenance/*.py`** (except `enforce_page_limit.py`):
        ...
        - **`Codex_Tasks/LinkedIn/*.py`**: Scripts for automating LinkedIn profile updates.
